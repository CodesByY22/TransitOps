...
export async function createTrip(prevState: any, formData: FormData): Promise<TripActionState> {
  ...
  try {
    // 1. Initial Validation Checks for assignments if they are set (even for Drafts, but critical on Dispatch)
    if (vehicleId) {
  ...
  try {
    ...
    if (nextStatus === "Completed") {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        // We will log a fuel log: cost is calculated based on liters (e.g. ₹95 per liter)
        ...
        // Log trip expenses (Toll cost = ₹120, Other misc = ₹150)
